...
//   static List<HttpClient> _clients = [];
//   static List<HttpClient> _activeClients = [];
...
//         await _client.get(_sanitizedUri(url, params), headers: _getHeaders(token: authenticate, userHeaders: headers)).timeout(Duration(seconds: TIME_OUT_DURATION));
...
//       {String method = 'POST', required Map<String, dynamic> body, Map<String, dynamic>? params, Map<String, String>? headers, bool authenticate = false}) async {
...
//     String payload = json.encode(body);
//     http.Response response =
//         await _client.put(_sanitizedUri(url, params), body: payload, headers: _getHeaders(token: authenticate, userHeaders: headers)).timeout(Duration(seconds: TIME_OUT_DURATION));
...
//     http.Response response = await _client.get(_sanitizedUri(url, {}), headers: _getHeaders(token: authenticate, userHeaders: {})).timeout(Duration(seconds: TIME_OUT_DURATION));
